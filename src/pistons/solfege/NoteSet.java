...
public class NoteSet extends Vector<NoteLearn> implements Serializable {
		...
		for (NoteName name : NoteName.values()) {
			for (NoteAccidental acc : NoteAccidental.values()) {
				for (int octaveNb = 2; octaveNb <= 4; octaveNb++) {
					if ((octaveNb == 2 && (name == NoteName.DO || name == NoteName.RE || name == NoteName.MI
						...
						add(new NoteLearn(acc, name, octaveNb));
					}
